...
def main(argv):
	...
	if not makemkvpath:
		...
		utils.exitFailed("{pleaseset} {pathtomakemkvcon}".format(pleaseset =
			getString(30072), pathtomakemkvcon = getString(30016)), "{pleaseset} "\
			"{pathtomakemkvcon}".format(pleaseset =	getString(30072), pathtomakemkvcon
			= getString(30016)))
	...
	for line in lines:
		...
		if len(drive) >= 7:
			if drive[5] != '""':
				gooddrives = gooddrives + drive[0].split(":")[1] + ": " +  \
					drive[5].replace('"','') + " "
